...
module.exports = {
  getApiKey: async function getApiKey() {
    ...
    await page.goto(omdbFreeApiSignUpUrl, {
    ...
    while (!omdbEmailExists) {
      ...
      omdbEmailExists = await page.evaluate(() => {
        let mostRecentEmail = document
          .querySelector("#schranka > tr")
          .children[0].childNodes[1].nodeValue.trim();
    ...
    let apiInfo = await page.evaluate(() => {
      ...
      var apiKey =
        innerDoc.children[0].children[1].childNodes[0].nodeValue.trim();
      apiKey = apiKey.substring(apiKey.indexOf(":") + 2);
      var verificationUrl =
        innerDoc.children[0].children[1].childNodes[6].nodeValue.trim();
      verificationUrl = verificationUrl.substring(
        verificationUrl.indexOf(":") + 2
      );
      let apiInfo = {
        apiKey: apiKey,
    ...
    await page.goto(apiInfo.verificationUrl, {
      waitUntil: "networkidle2",
      timeout: 0,
    });
